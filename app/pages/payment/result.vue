<template>
  <main class="result-page"><section class="result-panel">
    ...
    <div class="actions"><NuxtLink v-if="payment?.status === 'SUCCEEDED'" to="/trips">View my journeys</NuxtLink><NuxtLink v-else-if="retryableFailure" to="/orders">Try another payment method</NuxtLink><NuxtLink v-else to="/orders">View order status</NuxtLink></div>
  </section></main>
</template>
...
onMounted(async()=>{ if(!auth.member.value) await auth.loadMember(); await load() }); onBeforeUnmount(()=>{ if(timer) clearTimeout(timer) })
</script>
<style scoped>
