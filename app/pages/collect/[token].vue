<template>
  <main class="collection-page">
    <section class="collection-card">
      <header><span class="brand-mark">旅遇</span><p>安全收款</p></header>
      ...
      <template v-else-if="collection">
        <section class="summary">
          ...
          <p class="deadline">有效期至 {{ dateTime(collection.validUntil) }}</p>
        </section>
        ...
        <template v-else>
          ...
          <section v-if="bankChannel" class="method bank">
            ...
            <p class="bank-warning">请按页面金额一次性转账，并在附言中填写指定备注。无需上传转账凭证。</p>
          </section>
          <p v-if="!collection.channels.length" class="no-method">暂未配置可用付款方式，请联系收款方。</p>
        </template>
      </template>
      <footer>本页面由旅遇安全支付服务提供 · 请核对收款事由和金额</footer>
    </section>
