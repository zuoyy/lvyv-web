...
interface WishItem {
  ...
  interests: EnumLabel[]; budgetLevelLabel: string; story: string; status: number; statusLabel: string
...
const { auth, ready, initializeAccount } = useAccountPage('/wish')
...
const statusOptions = [
  { value: 10, label: 'Submitted' }, { value: 20, label: 'Designing' }, { value: 30, label: 'Delivered' },
  { value: 40, label: 'Revision requested' }, { value: 50, label: 'Revising' },
  { value: 80, label: 'Closed' }, { value: 90, label: 'Cancelled' },
]
...
const fetchWishes = async () => {
  ...
  try {
    const response = await $fetch<ApiResult<PageResult<WishItem>>>('/tour/wishes/page', {
      ...
      params: { page: page.value, size, ...(statusFilter.value ? { status: Number(statusFilter.value) } : {}) },
    })
...
const statusClass = (status: number) => status === 30 ? 'success' : status >= 80 ? 'muted' : status >= 40 ? 'warning' : 'active'
