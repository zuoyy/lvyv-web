  ...
  <AccountPageShell active-page="orders" kicker="Secure checkout" title="Pay for your journey" description="Choose a payment method and continue to Oceanpayment's secure hosted checkout." :ready="true" :show-navigation="false">
    ...
    <form v-else-if="order" class="payment-layout" @submit.prevent="submit">
      ...
      <aside class="payment-summary">
        ...
        <dl><div><dt>Journey</dt><dd>{{ order.items[0]?.snapshot?.title || 'Lvyv journey' }}</dd></div><div><dt>Adults</dt><dd>{{ order.items[0]?.item.adultCount || 1 }}</dd></div><div><dt>Children</dt><dd>{{ order.items[0]?.item.childCount || 0 }}</dd></div><div v-if="Number(order.order.pointsAmount) > 0"><dt>Points discount</dt><dd>- {{ order.order.currency }} {{ order.order.pointsAmount }}</dd></div></dl>
